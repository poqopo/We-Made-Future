...
contract We_Made_Future is ERC20("We_Made_FutureSwap", "WMF"), Ownable {
  ...
  event CapChanged(uint256 prevCap, uint256 newCap);s
  ...
  constructor(
    ...
    require(
      ...
      "We_Made_Future::constructor::_governor must not be address(0) or address(1)"
    );
    require(_endReleaseBlock > _startReleaseBlock, "We_Made_Future::constructor::endReleaseBlock < startReleaseBlock");
    _setupDecimals(18);
    cap = uint256(-1);
    governor = _governor;
  ...
  modifier onlyGovernor() {
    require(_msgSender() == governor, "We_Made_Future::onlyGovernor::not governor");
    _;
  }

  /// @dev Return unlocked We_Made_Future
  function unlockedSupply() external view returns (uint256) {
    return totalSupply().sub(totalLock());
  }

  /// @dev Return totalLocked We_Made_Future
  ...
  function setCap(uint256 _cap) external onlyGovernor {
    require(_cap < cap, "We_Made_Future::setCap::_cap must < cap");
    uint256 prevCap = cap;
    cap = _cap;
  ...
  function setGovernor(address _governor) external onlyGovernor {
    require(governor != _governor, "We_Made_Future::setGovernor::no self set");
    address prevGov = governor;
    governor = _governor;
    emit GovernorChanged(prevGov, governor);
  }

  /// @dev A function to mint We_Made_Future. This will be called by an owner only.
  /// @param _to The address of the account to get this newly mint We_Made_Future
  /// @param _amount The amount to be minted
  function mint(address _to, uint256 _amount) external onlyOwner {
    require(totalSupply().add(_amount) < cap, "We_Made_Future::mint::cap exceeded");
    _mint(_to, _amount);
  ...
  function transferFrom(
    ...
    _approve(
      _sender,
      _msgSender(),
      allowance(_sender, _msgSender()).sub(_amount, "We_Made_Future::transferFrom::transfer amount exceeds allowance")
    );
  ...
  /// @dev Return the locked We_Made_Future of a given account
  /// @param _account The address that you want to know the locked We_Made_Future
  ...
  /// @dev Lock We_Made_Future based-on the command from MasterChef
  ...
  function lock(address _account, uint256 _amount) external onlyOwner {
    require(_account != address(this), "We_Made_Future::lock::no lock to token address");
    require(_account != address(0), "We_Made_Future::lock::no lock to address(0)");
    require(_amount <= balanceOf(_account), "We_Made_Future::lock::no lock over balance");
  ...
  /// @dev Return how many We_Made_Future is unlocked for a given account
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no We_Made_Futures can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked We_Made_Futures can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some We_Made_Futures can be released
    else {
  ...
  /// @dev Claim unlocked We_Made_Future after the release schedule is reached
  function unlock() external {
    require(_locks[msg.sender] > 0, "We_Made_Future::unlock::no locked We_Made_Future");
  ...
  /// @dev Move both locked and unlocked We_Made_Future to another account
  /// @param _to The address to be received locked and unlocked We_Made_Future
  function transferAll(address _to) external {
    require(msg.sender != _to, "We_Made_Future::transferAll::no self-transferAll");
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "We_Made_Future::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "We_Made_Future::delegateBySig: invalid nonce");
    require(now <= expiry, "We_Made_Future::delegateBySig: signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
    require(blockNumber < block.number, "We_Made_Future::getPriorVotes: not yet determined");
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying We_Made_Futures (not scaled);
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "We_Made_Future::_writeCheckpoint: block number exceeds 32 bits");
